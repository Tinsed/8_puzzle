	...
	if(pNode->getDepth()<=problem->getMaxDepth()){
		...
		for(Node* i:*successors){
			...
			if(visitedNodes->contains(hash)){	//если в посещенных
				...
				visitedNodes->insert(hash,i->getDepth());//добавл€ем в посещенные
				if(stepMode)
...
Node* Tree_Search_DLS(Problem* problem, QHash<int,Node*> *unqNodes, QQueue<Node*>* fringe, QTextEdit* logWidget){
	...
	dNodes->insert(pStartNode->getState()->getHashI(),pStartNode->getDepth()); // ѕодсовываем его в посещенные
...
Node* Tree_Search_AStar(Problem* problem, QHash<int, Node*> *unqNodes, QQueue<Node*>* fringeo, QTextEdit* logWidget){
	...
	while(!fringe->isEmpty()){
		...
		QList<Node*>* successors = Expand(current, problem, problem->heuristicFoo); //раскрываем листь€
		//QString str = "Suc:";
